...
def load_data(): 
    #load data
    data = np.loadtxt('./spambase/spambase.data', delimiter=',', dtype=np.float64)
...
class NaiveBayesClassifier(object):
    ...
    def _gaussian(self, x, mu, sigma):
        ''' Returns the gaussian approx. of the class conditional probability P()
        Args:
...
def main():
    ...
    results = classifier.classify(data['test_data'])

    print('results ', results)
    print('accuracy = ', np.trace(results) / np.sum(results))
